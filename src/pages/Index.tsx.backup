...
const Index = () => {
  ...
  const coreExpertise = useMemo(
    () => [
      ...
      },
      {
        ...
        title: t("about.uiux"),
        description:
          "Design de interfaces intuitivas e experiências memoráveis",
  ...
  const handleProjectClick = useCallback(
    (projectTitle: string, url: string) => {
  ...
  const fetchGitHubData = useCallback(async () => {
    ...
    if (cachedData && cacheTimestamp) {
      const isDataFresh =
        Date.now() - parseInt(cacheTimestamp) < 60 * 60 * 1000; // 1 hour
      if (isDataFresh) {
        try {
    ...
    try {
      ...
      const techStackArray =
        languages.size > 0 ? Array.from(languages) : fallbackTechStack;
      ...
      const projects = repos.slice(0, 3).map((repo: GitHubRepo) => ({
        ...
        description:
          repo.description ||
          "Projeto de código aberto desenvolvido com foco em qualidade e performance.",
        technologies: [],
  ...
  useEffect(() => {
    // If we already have fresh cache, the fetchGitHubData will early-return inside
    // Defer network work until idle or user scrolls
    const start = () => fetchGitHubData();
    ...
    if (hasIdle) {
      ...
      // Fallback: wait for first scroll to indicate user intent
      window.addEventListener("scroll", onFirstScroll, {
        once: true,
        passive: true,
      });
      // Also set a timeout as a safety
      setTimeout(() => {
        window.removeEventListener("scroll", onFirstScroll);
        start();
      }, 4000);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        ...
        {/* Prefer inline skeletons; avoid blocking overlay to improve LCP */}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {coreExpertise.map((item, index) => (
                      <motion.div
                        key={item.title}
                        className={`p-4 rounded-xl bg-gradient-to-br ${item.color.replace(
                          "text-",
                          "from-"
                        )}/10 to-transparent border border-${item.color.replace(
                          "text-",
                          ""
                        )}/20 hover:shadow-lg transition-all duration-300 cursor-pointer`}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div
                          ...
                          title="bernardo.gomes@bebitterbebetter.com.br"
                        >
                          bernardo.gomes@bebitterbebetter.com.br
                        </div>
                      </div>
